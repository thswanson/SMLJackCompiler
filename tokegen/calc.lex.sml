...
fun yyAction24 (strm, lastMatch : yymatch) = let
      ...
      in
        yystrm := strm; (Tokens.StringConstant(yytext,!pos,!pos))
      end
       ...
       of NONE => yyAction14(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"+"
              ...
              then yyQ6(strm', yyMATCH(strm, yyAction14, yyNO_MATCH))
              else yyAction14(strm, yyNO_MATCH)
      (* end case *))
...
fun yyQ5 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction28(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ6(strm', yyMATCH(strm, yyAction28, yyNO_MATCH))
...
fun yyQ4 (strm, lastMatch : yymatch) = (case (yygetc(strm))
       of NONE => yyAction28(strm, yyNO_MATCH)
        | SOME(inp, strm') =>
            if inp = #"/"
              then yyQ8(strm', yyMATCH(strm, yyAction28, yyNO_MATCH))
              else yyAction28(strm, yyNO_MATCH)
      (* end case *))
...
in
  (case (!(yyss))
   of MULTI => yyQ0(!(yystrm), yyNO_MATCH)
